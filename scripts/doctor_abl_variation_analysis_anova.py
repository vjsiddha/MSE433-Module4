...
ABL_COUNT = "#ABL"
ABL_APPLICATIONS = "#APPLICATIONS 3/Abl"
...
def build_case_time_ancova(standard_cases: pd.DataFrame) -> pd.DataFrame:
    ...
    effects = ["C(PHYSICIAN)", f'Q("{ABL_COUNT}")', f'Q("{ABL_TIME}")']
    formula = f'Q("{CASE_TIME}") ~ ' + " + ".join(effects)
    model = smf.ols(formula, data=data).fit()
    return _case_time_anova_table(model, effects, len(data))


def build_case_time_ancova_extended(standard_cases: pd.DataFrame) -> pd.DataFrame:
    """Physician + #ABL + ABL TIME + applications per ablation (workload density)."""
    extra = [ABL_APPLICATIONS]
    cols = [CASE_TIME, "PHYSICIAN", ABL_COUNT, ABL_TIME, *extra]
    data = standard_cases.dropna(subset=cols).copy()
    terms = (
        ["C(PHYSICIAN)"]
        + [f'Q("{ABL_COUNT}")', f'Q("{ABL_TIME}")']
        + [f'Q("{c}")' for c in extra]
    )
    formula = f'Q("{CASE_TIME}") ~ ' + " + ".join(terms)
    model = smf.ols(formula, data=data).fit()
    return _case_time_anova_table(model, terms, len(data))


def build_case_time_ancova_preamp_controls(standard_cases: pd.DataFrame) -> pd.DataFrame:
    """Same core predictors plus pre-ablation phase minutes (prep, access, TSP, pre-map)."""
    preamp = [PT_PREP, ACCESS, TSP, PRE_MAP]
    cols = [CASE_TIME, "PHYSICIAN", ABL_COUNT, ABL_TIME, *preamp]
    data = standard_cases.dropna(subset=cols).copy()
    terms = (
        ["C(PHYSICIAN)"]
        + [f'Q("{c}")' for c in preamp]
        + [f'Q("{ABL_COUNT}")', f'Q("{ABL_TIME}")']
    )
    formula = f'Q("{CASE_TIME}") ~ ' + " + ".join(terms)
    model = smf.ols(formula, data=data).fit()
...
def main() -> None:
    ...
    case_time_ancova_ext = build_case_time_ancova_extended(standard_cases)
    case_time_ancova_preamp = build_case_time_ancova_preamp_controls(standard_cases)
    ...
    case_time_ancova_ext.to_csv(OUTPUT_DIR / "case_time_ancova_extended.csv", index=False)
    case_time_ancova_preamp.to_csv(OUTPUT_DIR / "case_time_ancova_preamp_controls.csv", index=False)
    ...
    print("-", OUTPUT_DIR / "case_time_ancova_extended.csv")
    print("-", OUTPUT_DIR / "case_time_ancova_preamp_controls.csv")
    ...
    print()
    print("Case-time ANCOVA (extended: + applications/Abl)")
    print(case_time_ancova_ext.to_string(index=False))
    print()
    print("Case-time ANCOVA (+ prep, access, TSP, pre-map)")
    print(case_time_ancova_preamp.to_string(index=False))
    print()
